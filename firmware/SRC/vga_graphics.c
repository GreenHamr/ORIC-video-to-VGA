...
char * getFrameBufferAddressPointer() {
  return &vga_data_array[0];
}

#define CORR_Y 42
void copyLineToFrameBuffer(int Line,unsigned char * Source , uint8_t Count) {
        uint32_t address = (Line * 640);
        uint32_t address_S = ((Line+CORR_Y) * 240);
        memcpy(&vga_data_array[address],&Source[address_S],240);
        memcpy(&vga_data_array[address+320],&Source[address_S],240);
        
}

// 320 x 240
void drawPixel2(short x, short y, char color) {
    int pixel = ((320 * y * 2) + x) ; 
    int pixel2 = ((320 * y * 2) + x) + 640 ; 
    vga_data_array[pixel>>1] = (color & 7) | ((color  & 7 )<< 3) ;
    vga_data_array[pixel2>>1] = (color & 7) | ((color  & 7 )<< 3) ;
}


void writeFrameBufferAddress(uint32_t address,uint8_t data) {
  vga_data_array[address] = data;
}
